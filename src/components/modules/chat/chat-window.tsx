...
export function ChatWindow() {
  ...
  if (state.error) {
    return <ErrorState message={state.error} onRetry={() => setState(prev => ({ ...prev, error: null }))} />
  }
          ...
          {state.chats.map(chat => (
            <div
              ...
              className={cn(
                ...
                state.activeChat?.id === chat.id && 'bg-gray-100 dark:bg-[#1E3A5F]/50'
              )}
              onClick={() => {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <p className="font-medium truncate dark:text-white text-gray-900">{chat.name}</p>
                  <span className="text-xs dark:text-gray-400 text-gray-500">
                    {chat.messages[chat.messages.length - 1]?.timestamp}
                  </span>
                </div>
                <div className="flex items-center gap-1">
                  <p className="text-sm truncate dark:text-gray-400 text-gray-600">{chat.lastMessage}</p>
        ...
        {state.activeChat ? (
          <>
            ...
            <div className="flex items-center gap-3 p-4 border-b dark:border-[#1E3A5F] border-gray-200">
              ...
              <div>
                <h3 className="font-medium dark:text-white text-gray-900">{state.activeChat.name}</h3>
