...
export function ApprovalConfirmationModal({
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Failed to approve milestone")
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !isLoading && !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            You are about to approve milestone {Number.parseInt(milestoneIndex) + 1} for escrow &quot;{escrow.title}&quot;.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4">
          <div className="space-y-3">
            <div className="p-3 border rounded-md bg-muted/50">
              ...
              <p className="text-sm">{escrow.milestones[Number.parseInt(milestoneIndex)]?.description}</p>
            </div>

            <div className="text-sm space-y-2">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Contract ID:</span>
                <span className="font-mono truncate max-w-[200px]">{escrow.contractId}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Service Provider:</span>
                <span className="font-mono truncate max-w-[200px]">{escrow.serviceProvider}</span>
              </div>
            </div>
        ...
        <DialogFooter className="flex sm:justify-between">
          ...
          <Button type="button" onClick={handleApprove} disabled={isLoading || success}>
            {isLoading ? (
              <>
