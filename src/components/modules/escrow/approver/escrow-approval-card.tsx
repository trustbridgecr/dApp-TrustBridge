...
export function EscrowApprovalCard({ escrow, onApprove }: EscrowApprovalCardProps) {
  ...
  return (
    <Card className="h-full flex flex-col">
      <CardHeader>
        ...
        <CardDescription className="line-clamp-2">{escrow.description}</CardDescription>
      </CardHeader>
          ...
          <div>
            ...
            <div className="space-y-2">
              {pendingMilestones.map((milestone) => (
                <div key={milestone.index} className="p-3 border rounded-md bg-muted/50">
                  ...
                  <p className="text-sm line-clamp-2">{milestone.description}</p>
                  <Button
                    variant="default"
                    size="sm"
                    className="w-full mt-2 cursor-pointer"
                    onClick={() => onApprove(escrow, milestone.index.toString())}
                  >
