...
export function useApproveEscrow() {
    ...
    const [selectedMilestoneIndex, setSelectedMilestoneIndex] = useState<string | null>(null)
        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                toast.error("Failed to fetch escrows", {
                    description: response.message || "An error occurred while fetching escrows.",
                })
            }
        } catch (error) {
        ...
        const fetchData = async () => {
            try {
                ...
                toast.error("Wallet Error", {
                    description: "Could not get wallet address. Please make sure your wallet is connected.",
                })
                setLoading(false)
            }
        }
