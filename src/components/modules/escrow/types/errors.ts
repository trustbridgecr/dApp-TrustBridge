...
export const ESCROW_RELEASE_ERRORS: Record<EscrowReleaseErrorType, string> = {
  ...
  MILESTONE_INCOMPLETE: 'All milestones must be completed before releasing funds',
...
export function isEscrowReleaseError(error: unknown): error is EscrowReleaseError {
  return (
    typeof error === 'object' &&
    error !== null &&
    'type' in error &&
